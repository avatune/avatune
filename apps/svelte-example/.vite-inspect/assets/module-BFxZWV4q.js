...
import {
  ...
  withCtx,
  withDirectives,
} from './runtime-core.esm-bundler-Cyv4obHQ.js'
...
const Pe = {
    ...
    setup(D, { emit: h$1 }) {
      const y = h$1,
        ...
        _ = computed(() => ({
          [`splitpanes splitpanes--${u.horizontal ? 'horizontal' : 'vertical'}`]:
            !0,
  ...
  ge = {
    ...
    setup(D) {
      ...
      const h$1 = D,
        ...
        f = computed(() => {
          const r =
            isNaN(h$1.size) || h$1.size === void 0 ? 0 : parseFloat(h$1.size)
  ...
  ;(function (global, factory) {
    ...
    function elt(tag, content, className, style) {
      ...
      if (typeof content == 'string')
        ...
        for (var i$3 = 0; i$3 < content.length; ++i$3)
          e.appendChild(content[i$3])
      return e
    }
    ...
    function joinClasses(a, b) {
      var as = a.split(' ')
      for (var i$3 = 0; i$3 < as.length; i$3++)
        if (as[i$3] && !classTest(as[i$3]).test(b)) b += ' ' + as[i$3]
      return b
    }
    ...
    function copyObj(obj, target, overwrite) {
      ...
      for (var prop$1 in obj)
        if (
          obj.hasOwnProperty(prop$1) &&
          (overwrite !== false || !target.hasOwnProperty(prop$1))
        )
    ...
    var bidiOrdering = (function () {
      ...
      return function (str, direction) {
        ...
        for (var i$3 = 0; i$3 < len; ++i$3)
          types.push(charType(str.charCodeAt(i$3)))
        ...
        for (var i$3$1 = 1, prev$1 = types[0]; i$3$1 < len - 1; ++i$3$1) {
          ...
          if (type$2 == '+' && prev$1 == '1' && types[i$3$1 + 1] == '1')
            types[i$3$1] = '1'
          else if (
            type$2 == ',' &&
            prev$1 == types[i$3$1 + 1] &&
            (prev$1 == '1' || prev$1 == 'n')
          )
            types[i$3$1] = prev$1
          prev$1 = type$2
    ...
    function signal(emitter, type) {
      ...
      for (var i$3 = 0; i$3 < handlers.length; ++i$3)
        handlers[i$3].apply(null, args)
    }
    ...
    function signalCursorActivity(cm) {
      ...
      var set =
        ...
        (cm.curOp.cursorActivityHandlers = [])
      for (var i$3 = 0; i$3 < arr.length; ++i$3)
        if (indexOf(set, arr[i$3]) == -1) set.push(arr[i$3])
    }
    ...
    function getMode(options, spec) {
      ...
      if (modeExtensions.hasOwnProperty(spec.name)) {
        ...
        for (var prop$1 in exts) {
          ...
          if (modeObj.hasOwnProperty(prop$1))
            modeObj['_' + prop$1] = modeObj[prop$1]
      ...
      if (spec.modeProps)
        for (var prop$1$1 in spec.modeProps)
          modeObj[prop$1$1] = spec.modeProps[prop$1$1]
      return modeObj
    }
    var modeExtensions = {}
    ...
    function getLine(doc$1, n) {
      ...
      if (n < 0 || n >= doc$1.size)
        throw new Error(
          'There is no line ' + (n + doc$1.first) + ' in the document.',
        )
      var chunk = doc$1
      while (!chunk.lines)
        for (var i$3 = 0; ; ++i$3) {
    ...
    function clipPosArray(doc$1, array) {
      var out = []
      for (var i$3 = 0; i$3 < array.length; i$3++)
        out[i$3] = clipPos(doc$1, array[i$3])
      return out
    }
    ...
    Context.fromSaved = function (doc$1, saved, line) {
      if (saved instanceof SavedContext)
        return new Context(
          doc$1,
          copyState(doc$1.mode, saved.state),
          line,
          saved.lookAhead,
        )
    ...
    function takeToken(cm, pos, precise, asArray) {
      ...
      while ((asArray || stream.pos < pos.ch) && !stream.eol()) {
        ...
        if (asArray)
          tokens.push(
            new Token(stream, style, copyState(doc$1.mode, context.state)),
          )
      }
    ...
    function removeMarkedSpan(spans, span) {
      var r
      for (var i$3 = 0; i$3 < spans.length; ++i$3)
        if (spans[i$3] != span) (r || (r = [])).push(spans[i$3])
      return r
    }
    ...
    function stretchSpansOverChange(doc$1, change) {
      ...
      var oldFirst =
        isLine(doc$1, change.from.line) &&
        getLine(doc$1, change.from.line).markedSpans
      var oldLast =
        isLine(doc$1, change.to.line) &&
        getLine(doc$1, change.to.line).markedSpans
          ...
          for (var i$2$1 = 0; i$2$1 < first.length; ++i$2$1)
            if (first[i$2$1].to == null)
              (gapMarkers || (gapMarkers = [])).push(
                new MarkedSpan(first[i$2$1].marker, null, null),
              )
        }
    ...
    function detachMarkedSpans(line) {
      ...
      if (!spans) return
      for (var i$3 = 0; i$3 < spans.length; ++i$3)
        spans[i$3].marker.detachLine(line)
      line.markedSpans = null
    }
    function attachMarkedSpans(line, spans) {
      if (!spans) return
      for (var i$3 = 0; i$3 < spans.length; ++i$3)
        spans[i$3].marker.attachLine(line)
    ...
    function buildLineContent(cm, lineView) {
      ...
      for (
        var i$3 = 0;
        i$3 <= (lineView.rest ? lineView.rest.length : 0);
        i$3++
      ) {
    ...
    function splitSpaces(text, trailingBefore) {
      ...
      for (var i$3 = 0; i$3 < text.length; i$3++) {
        ...
        if (
          ch == ' ' &&
          spaceBefore &&
          (i$3 == text.length - 1 || text.charCodeAt(i$3 + 1) == 32)
        )
          ch = '\xA0'
        result += ch
        spaceBefore = ch == ' '
    ...
    function updateLineBackground(cm, lineView) {
      ...
      if (lineView.background)
        ...
        lineView.background = wrap$1.insertBefore(
          elt('div', null, cls),
          wrap$1.firstChild,
        )
    ...
    function insertLineWidgetsFor(cm, line, lineView, dims, allowAbove) {
      ...
      for (var i$3 = 0, ws = line.widgets; i$3 < ws.length; ++i$3) {
        ...
        if (allowAbove && widget.above)
          wrap$1.insertBefore(node, lineView.gutter || lineView.text)
      ...
      for (var i$3 = 0; i$3 < map$1.length; i$3 += 3) {
        ...
        if (start != null) {
          ...
          if (bias == 'left' && start == 0)
            while (
              i$3 &&
              map$1[i$3 - 2] == map$1[i$3 - 3] &&
              map$1[i$3 - 1].insertLeft
            ) {
              node = map$1[(i$3 -= 3) + 2]
              collapse = 'left'
            }
    ...
    function getUsefulRect(rects, bias) {
      var rect = nullRect
      if (bias == 'left') {
        for (var i$3 = 0; i$3 < rects.length; i$3++)
          if ((rect = rects[i$3]).left != rect.right) break
      } else
    ...
    function clearLineMeasurementCache(cm) {
      ...
      for (var i$3 = 0; i$3 < cm.display.view.length; i$3++)
        clearLineMeasurementCacheFor(cm.display.view[i$3])
    }
    ...
    function pageScrollX(doc$1) {
      if (chrome && android)
        return -(
          doc$1.body.getBoundingClientRect().left -
          parseInt(getComputedStyle(doc$1.body).marginLeft)
        )
      return (
        doc$1.defaultView.pageXOffset ||
        (doc$1.documentElement || doc$1.body).scrollLeft
      )
    }
    function pageScrollY(doc$1) {
      if (chrome && android)
        return -(
          doc$1.body.getBoundingClientRect().top -
          parseInt(getComputedStyle(doc$1.body).marginTop)
        )
      return (
        doc$1.defaultView.pageYOffset ||
        (doc$1.documentElement || doc$1.body).scrollTop
      )
    }
    ...
    function coordsChar(cm, x, y) {
      ...
      if (lineN > last)
        return PosWithInfo(
          doc$1.first + doc$1.size - 1,
          getLine(doc$1, last).text.length,
          null,
          1,
          1,
        )
      ...
      return function (line) {
        ...
        if (line.widgets) {
          for (var i$3 = 0; i$3 < line.widgets.length; i$3++)
            if (line.widgets[i$3].height)
              widgetsHeight += line.widgets[i$3].height
        }
        if (wrapping)
          return (
    ...
    function visibleLines(display, doc$1, viewport) {
      ...
      if (viewport && viewport.ensure) {
        ...
        if (ensureFrom < from) {
          ...
          to = lineAtHeight(
            doc$1,
            heightAtLine(getLine(doc$1, ensureFrom)) +
              display.wrapper.clientHeight,
          )
        } else if (Math.min(ensureTo, doc$1.lastLine()) >= to) {
          from = lineAtHeight(
            doc$1,
            heightAtLine(getLine(doc$1, ensureTo)) -
              display.wrapper.clientHeight,
          )
          to = ensureTo
        }
    ...
    function maybeScrollWindow(cm, rect) {
      ...
      if (rect.top + box.top < 0) doScroll = true
        ...
        rect.bottom + box.top >
        (doc$1.defaultView.innerHeight || doc$1.documentElement.clientHeight)
      )
        doScroll = false
    ...
    function calculateScrollPos(cm, rect) {
      ...
      if (rect.top < screentop) result.scrollTop = atTop ? 0 : rect.top
        ...
        var newTop = Math.min(
          rect.top,
          (atBottom ? docBottom : rect.bottom) - screen$1,
        )
    ...
    function updateScrollbars(cm, measure) {
      ...
      for (
        var i$3 = 0;
        (i$3 < 4 && startWidth != cm.display.barWidth) ||
        startHeight != cm.display.barHeight;
        i$3++
      ) {
    ...
    function endOperation(cm) {
      ...
      if (op)
        finishOperation(op, function (group) {
          for (var i$3 = 0; i$3 < group.ops.length; i$3++)
            group.ops[i$3].cm.curOp = null
          endOperations(group)
        })
    }
    function endOperations(group) {
      ...
      for (var i$1$1 = 0; i$1$1 < ops.length; i$1$1++)
        endOperation_W1(ops[i$1$1])
      for (var i$2$1 = 0; i$2$1 < ops.length; i$2$1++)
        endOperation_R2(ops[i$2$1])
      for (var i$3$1 = 0; i$3$1 < ops.length; i$3$1++)
        endOperation_W2(ops[i$3$1])
    ...
    function endOperation_finish(op) {
      ...
      if (display.wrapper.offsetHeight)
        doc$1.scrollTop = cm.display.scroller.scrollTop
    ...
    function highlightWorker(cm) {
      ...
      doc$1.iter(
        context.line,
        Math.min(doc$1.first + doc$1.size, cm.display.viewTo + 500),
        function (line) {
          if (context.line >= cm.display.viewFrom) {
            var oldStyles = line.styles
            var resetState =
              line.text.length > cm.options.maxHighlightLength
                ? copyState(doc$1.mode, context.state)
                : null
      ...
      if (changedLines.length)
        runInOp(cm, function () {
          for (var i$3 = 0; i$3 < changedLines.length; i$3++)
            regLineChange(cm, changedLines[i$3], 'text')
        })
    }
    ...
    DisplayUpdate.prototype.finish = function () {
      for (var i$3 = 0; i$3 < this.events.length; i$3++)
        signal.apply(null, this.events[i$3])
    }
    ...
    function updateDisplayIfNeeded(cm, update) {
      ...
      if (display.viewFrom < from && from - display.viewFrom < 20)
        from = Math.max(doc$1.first, display.viewFrom)
    ...
    function patchDisplay(cm, updateNumbersFrom, dims) {
      ...
      function rm(node$1) {
        ...
        if (webkit && mac && cm.display.currentWheelTarget == node$1)
      ...
      for (var i$3 = 0; i$3 < view.length; i$3++)
        if (!view[i$3].hidden) {
          if (cm.options.fixedGutter) {
            ...
            if (view[i$3].gutterBackground)
              view[i$3].gutterBackground.style.left = left
          }
    ...
    Selection.prototype.deepCopy = function () {
      ...
      for (var i$3 = 0; i$3 < this.ranges.length; i$3++)
        out[i$3] = new Range(
          copyPos(this.ranges[i$3].anchor),
          copyPos(this.ranges[i$3].head),
        )
      return new Selection(out, this.primIndex)
    }
    Selection.prototype.somethingSelected = function () {
      for (var i$3 = 0; i$3 < this.ranges.length; i$3++)
        if (!this.ranges[i$3].empty()) return true
      return false
    }
    Selection.prototype.contains = function (pos, end) {
      ...
      for (var i$3 = 0; i$3 < this.ranges.length; i$3++) {
        ...
        if (cmp(end, range$1.from()) >= 0 && cmp(pos, range$1.to()) <= 0)
          return i$3
      }
      return -1
    }
    ...
    function computeSelAfterChange(doc$1, change) {
      ...
      for (var i$3 = 0; i$3 < doc$1.sel.ranges.length; i$3++) {
        ...
        out.push(
          new Range(
            adjustForChange(range$1.anchor, change),
            adjustForChange(range$1.head, change),
          ),
        )
      }
    ...
    function addChangeToHistory(doc$1, change, selAfter, opId) {
      ...
      if (
        (hist.lastOp == opId ||
          (hist.lastOrigin == change.origin &&
            ...
            ((change.origin.charAt(0) == '+' &&
              hist.lastModTime >
                time - (doc$1.cm ? doc$1.cm.options.historyEventDelay : 500)) ||
              change.origin.charAt(0) == '*'))) &&
        ...
        if (!before || !before.ranges)
          pushSelectionToHistory(doc$1.sel, hist.done)
        cur = {
    ...
    function selectionEventCanBeMerged(doc$1, origin, prev, sel) {
      ...
      return (
        ...
        (ch == '+' &&
          ...
          new Date() - doc$1.history.lastSelTime <=
            (doc$1.cm ? doc$1.cm.options.historyEventDelay : 500))
      )
    }
    ...
    function attachLocalSpans(doc$1, change, from, to) {
      var existing = change['spans_' + doc$1.id],
        n = 0
      doc$1.iter(
        Math.max(doc$1.first, from),
        Math.min(doc$1.first + doc$1.size, to),
        function (line) {
          if (line.markedSpans)
            (existing || (existing = change['spans_' + doc$1.id] = {}))[n] =
              line.markedSpans
          ++n
        },
      )
    ...
    function getOldSpans(doc$1, change) {
      ...
      var nw = []
      for (var i$3 = 0; i$3 < change.text.length; ++i$3)
        nw.push(removeClearedSpans(found[i$3]))
      return nw
    }
    ...
    function extendSelection(doc$1, head, other, options, extend) {
      if (extend == null)
        extend = doc$1.cm && (doc$1.cm.display.shift || doc$1.extend)
      setSelection(
        doc$1,
        new Selection(
          [extendRange(doc$1.sel.primary(), head, other, extend)],
          0,
        ),
        options,
      )
    ...
    function replaceOneSelection(doc$1, i$3, range$1, options) {
      ...
      setSelection(
        doc$1,
        normalizeSelection(doc$1.cm, ranges, doc$1.sel.primIndex),
        options,
      )
    }
    ...
    function filterSelectionChange(doc$1, sel, options) {
      ...
      if (obj.ranges != sel.ranges)
        return normalizeSelection(doc$1.cm, obj.ranges, obj.ranges.length - 1)
      else return sel
    }
    ...
    function setSelection(doc$1, sel, options) {
      ...
      addSelectionToHistory(
        doc$1,
        doc$1.sel,
        doc$1.cm ? doc$1.cm.curOp.id : NaN,
        options,
      )
    }
    ...
    function reCheckSelection(doc$1) {
      setSelectionInner(
        doc$1,
        skipAtomicInSelection(doc$1, doc$1.sel, null, false),
      )
    }
    function skipAtomicInSelection(doc$1, sel, bias, mayClear) {
      ...
      for (var i$3 = 0; i$3 < sel.ranges.length; i$3++) {
        ...
        var old =
          sel.ranges.length == doc$1.sel.ranges.length && doc$1.sel.ranges[i$3]
    ...
    function movePos(doc$1, pos, dir, line) {
      if (dir < 0 && pos.ch == 0)
        ...
        dir > 0 &&
        pos.ch == (line || getLine(doc$1, pos.line)).text.length
      )
    ...
    function makeChange(doc$1, change, ignoreReadOnly) {
      if (doc$1.cm) {
        if (!doc$1.cm.curOp)
          return operation(doc$1.cm, makeChange)(doc$1, change, ignoreReadOnly)
        if (doc$1.cm.state.suppressEdits) return
      }
      if (
        hasHandler(doc$1, 'beforeChange') ||
        (doc$1.cm && hasHandler(doc$1.cm, 'beforeChange'))
      ) {
      ...
      var split =
        sawReadOnlySpans &&
        !ignoreReadOnly &&
        removeReadOnlyRanges(doc$1, change.from, change.to)
      if (split)
    ...
    function makeChangeInner(doc$1, change) {
      ...
      makeChangeSingleDoc(
        ...
        change,
        selAfter,
        stretchSpansOverChange(doc$1, change),
      )
      var rebased = []
      linkedDocs(doc$1, function (doc$2, sharedHist) {
        ...
        makeChangeSingleDoc(
          ...
          change,
          null,
          stretchSpansOverChange(doc$2, change),
        )
      })
    }
    function makeChangeFromHistory(doc$1, type, allowSelectionOnly) {
      ...
      for (; i$3 < source.length; i$3++) {
        ...
        if (
          allowSelectionOnly
            ? event.ranges && !event.equals(doc$1.sel)
            : !event.ranges
        )
          break
      }
    ...
    function shiftDoc(doc$1, distance) {
      ...
      if (doc$1.cm) {
        ...
        for (var d = doc$1.cm.display, l = d.viewFrom; l < d.viewTo; l++)
          regLineChange(doc$1.cm, l, 'gutter')
      }
    }
    function makeChangeSingleDoc(doc$1, change, selAfter, spans) {
      if (doc$1.cm && !doc$1.cm.curOp)
        return operation(doc$1.cm, makeChangeSingleDoc)(
          doc$1,
          change,
          selAfter,
          spans,
      ...
      if (doc$1.cantEdit && skipAtomic(doc$1, Pos(doc$1.firstLine(), 0)))
        doc$1.cantEdit = false
    }
    function makeChangeSingleDocInEditor(cm, change, spans) {
      ...
      if (doc$1.sel.contains(change.from, change.to) > -1)
    ...
    function changeLine(doc$1, handle, changeType, op) {
      ...
      if (typeof handle == 'number')
        line = getLine(doc$1, clipLine(doc$1, handle))
    ...
    BranchChunk.prototype = {
      ...
      collapse: function (lines) {
        for (var i$3 = 0; i$3 < this.children.length; ++i$3)
          this.children[i$3].collapse(lines)
      },
      insertInner: function (at, lines, height) {
    ...
    LineWidget.prototype.clear = function () {
      ...
      for (var i$3 = 0; i$3 < ws.length; ++i$3)
        if (ws[i$3] == this) ws.splice(i$3--, 1)
    ...
    function markText(doc$1, from, to, options, type) {
      if (options && options.shared)
        return markTextShared(doc$1, from, to, options, type)
      if (doc$1.cm && !doc$1.cm.curOp)
        return operation(doc$1.cm, markText)(doc$1, from, to, options, type)
      ...
      if (marker.collapsed) {
        if (
          ...
          (from.line != to.line &&
            conflictingCollapsedRange(doc$1, to.line, from, to, marker))
        )
          throw new Error(
      ...
      if (marker.readOnly) {
        seeReadOnlySpans()
        if (doc$1.history.done.length || doc$1.history.undone.length)
          doc$1.clearHistory()
      }
      if (marker.collapsed) {
      ...
      if (cm) {
        ...
        if (marker.collapsed) regChange(cm, from.line, to.line + 1)
          ...
          for (var i$3 = from.line; i$3 <= to.line; i$3++)
            regLineChange(cm, i$3, 'text')
    ...
    SharedTextMarker.prototype.clear = function () {
      if (this.explicitlyCleared) return
      this.explicitlyCleared = true
      for (var i$3 = 0; i$3 < this.markers.length; ++i$3)
        this.markers[i$3].clear()
    ...
    function markTextShared(doc$1, from, to, options, type) {
      ...
      linkedDocs(doc$1, function (doc$2) {
        ...
        for (var i$3 = 0; i$3 < doc$2.linked.length; ++i$3)
          if (doc$2.linked[i$3].isParent) return
        primary = lst(markers)
      })
      return new SharedTextMarker(markers, primary)
    }
    function findSharedMarkers(doc$1) {
      return doc$1.findMarks(
        Pos(doc$1.first, 0),
        doc$1.clipPos(Pos(doc$1.lastLine())),
        function (m) {
          return m.parent
        },
      )
    ...
    Doc.prototype = createObj(BranchChunk.prototype, {
      ...
      replaceSelections: docMethodOp(function (code, collapse, origin) {
        ...
        for (var i$1$1 = changes.length - 1; i$1$1 >= 0; i$1$1--)
          makeChange(this, changes[i$1$1])
      ...
      historySize: function () {
        var hist = this.history,
          ...
          undone = 0
        for (var i$3 = 0; i$3 < hist.done.length; i$3++)
          if (!hist.done[i$3].ranges) ++done
        for (var i$1$1 = 0; i$1$1 < hist.undone.length; i$1$1++)
          if (!hist.undone[i$1$1].ranges) ++undone
        return {
          undo: done,
          redo: undone,
        }
    ...
    function onDrop(e) {
      ...
      if (files && files.length && window.FileReader && window.File) {
        ...
        for (var i$3 = 0; i$3 < files.length; i$3++)
          readTextFromFile(files[i$3], i$3)
      } else {
    ...
    function moveVisually(cm, line, start, dir) {
      ...
      var wrappedLineExtent$1 = getWrappedLineExtent(
      ...
      var searchInVisualLine = function (
        ...
        for (; partPos$1 >= 0 && partPos$1 < bidi.length; partPos$1 += dir$1) {
          ...
          var ch$1 = moveInStorageOrder$1
            ? wrappedLineExtent$2.begin
            : mv(wrappedLineExtent$2.end, -1)
          if (part$1.from <= ch$1 && ch$1 < part$1.to)
            return getRes(ch$1, moveInStorageOrder$1)
          ch$1 = moveInStorageOrder$1 ? part$1.from : mv(part$1.to, -1)
          if (
            wrappedLineExtent$2.begin <= ch$1 &&
            ch$1 < wrappedLineExtent$2.end
          )
            return getRes(ch$1, moveInStorageOrder$1)
        }
      }
      ...
      if (res) return res
      var nextCh =
        dir > 0 ? wrappedLineExtent$1.end : mv(wrappedLineExtent$1.begin, -1)
      ...
      function extendTo(pos) {
        ...
        if (behavior.unit == 'rectangle') {
          ...
          for (
            ...
            if (left == right)
              ranges$1.push(new Range(Pos(line, leftPos), Pos(line, leftPos)))
          ...
          setSelection(
            ...
            normalizeSelection(
              cm,
              startSel.ranges.slice(0, ourIndex).concat(ranges$1),
              ourIndex,
            ),
            {
              origin: '*mouse',
          ...
          ranges$1$1[ourIndex] = bidiSimplify(
            cm,
            new Range(clipPos(doc$1, anchor), head),
          )
          setSelection(
            doc$1,
            normalizeSelection(cm, ranges$1$1, ourIndex),
            sel_mouse,
          )
        }
      }
      ...
      option('lineSeparator', null, function (cm, val) {
        ...
        for (var i$3 = newBreaks.length - 1; i$3 >= 0; i$3--)
          replaceRange(
            ...
            val,
            newBreaks[i$3],
            Pos(newBreaks[i$3].line, newBreaks[i$3].ch + val.length),
          )
      })
      option(
        'specialChars',
    ...
    function indentLine(cm, n, how, aggressive) {
      ...
      if (how == 'prev')
        if (n > doc$1.first)
          indentation = countColumn(getLine(doc$1, n - 1).text, null, tabSize)
        else indentation = 0
      ...
      if (indentString != curSpaceString) {
        ...
        for (var i$1$1 = 0; i$1$1 < doc$1.sel.ranges.length; i$1$1++) {
          ...
          if (
            range$1.head.line == n &&
            range$1.head.ch < curSpaceString.length
          ) {
        ...
        if (range$1.empty()) {
          if (deleted && deleted > 0) from = Pos(from.line, from.ch - deleted)
            ...
            to = Pos(
              to.line,
              Math.min(
                getLine(doc$1, to.line).text.length,
    ...
    function triggerElectric(cm, inserted) {
      ...
      for (var i$3 = sel.ranges.length - 1; i$3 >= 0; i$3--) {
        ...
        if (
          range$1.head.ch > 100 ||
          (i$3 && sel.ranges[i$3 - 1].head.line == range$1.head.line)
        )
          continue
    ...
    function addEditorMethods(CodeMirror$2) {
      ...
      CodeMirror$2.prototype = {
        ...
        addOverlay: methodOp(function (spec, options) {
          var mode = spec.token
            ? spec
            : CodeMirror$2.getMode(this.options, spec)
        ...
        getStateAfter: function (line, precise) {
          ...
          line = clipLine(
            doc$1,
            line == null ? doc$1.first + doc$1.size - 1 : line,
          )
        ...
        deleteH: methodOp(function (dir, unit) {
          ...
          if (sel.somethingSelected())
            doc$1.replaceSelection('', null, '+delete')
          else
      ...
      CodeMirror$2.registerHelper = function (type, name, value) {
        if (!helpers.hasOwnProperty(type))
          helpers[type] = CodeMirror$2[type] = { _global: [] }
        helpers[type][name] = value
      }
      CodeMirror$2.registerGlobalHelper = function (
        type,
        name,
        predicate,
        value,
      ...
      function moveOnce(boundToLine) {
        ...
        if (unit == 'codepoint') {
          ...
          if (isNaN(ch)) next = null
            ...
            var astral =
              dir > 0 ? ch >= 55296 && ch < 56320 : ch >= 56320 && ch < 57343
            next = new Pos(
              pos.line,
              Math.max(
                0,
        ...
        if (next == null)
          if (!boundToLine && findNextLine())
            pos = endOfLine(visually, doc$1.cm, lineObj, pos.line, lineDir)
          else return false
        else pos = next
        return true
      }
      ...
      function walk(node) {
        if (node.nodeType == 1) {
          ...
          if (!/^br$/i.test(node.nodeName) && node.textContent.length == 0)
            return
          if (isBlock) close()
          for (var i$3 = 0; i$3 < node.childNodes.length; i$3++)
            walk(node.childNodes[i$3])
        ...
        for (var i$3 = -1; i$3 < (maps ? maps.length : 0); i$3++) {
          ...
          for (var j = 0; j < map$1.length; j += 3) {
            ...
            if (curNode == textNode$1 || curNode == topNode$1) {
              ...
              if (offset$1 < 0 || curNode != textNode$1)
                ch = map$1[j + (offset$1 ? 1 : 0)]
    ...
    CodeMirror$1.defineExtension(
      ...
      function (template, callback, options) {
        ...
        if (inp) {
          ...
          if (options.closeOnBlur !== false)
            CodeMirror$1.on(button, 'blur', close)
          button.focus()
        }
        return close
      },
...
var require_searchcursor = __commonJSMin((exports, module) => {
  ;(function (mod) {
    ...
    SearchCursor.prototype = {
      ...
      find: function (reverse) {
        ...
        if (this.afterEmptyMatch && this.atOccurrence) {
          ...
          if (CodeMirror$1.cmpPos(head, this.doc.clipPos(head)) != 0)
            return (this.atOccurrence = false)
        }
        var result = this.matches(reverse, head)
        this.afterEmptyMatch =
          result && CodeMirror$1.cmpPos(result.from, result.to) == 0
        if (result) {
          this.pos = result
    ...
    CodeMirror$1.defineDocExtension(
      'getSearchCursor',
      function (query, pos, caseFold) {
        ...
        var searchNext = function (query, event) {
          ...
          findNext(cm, event.shiftKey, function (_, to) {
            ...
            if (
              ...
              dialog$1.getBoundingClientRect().bottom - 4 >
        ...
        persistentDialog(
          ...
          function (event, query) {
            ...
            var extra = cm.getOption('extraKeys'),
              cmd =
                (extra && extra[keyName]) ||
                CodeMirror$1.keyMap[cm.getOption('keyMap')][keyName]
            if (
              cmd == 'findNext' ||
              cmd == 'findPrev' ||
      ...
      cm.operation(function () {
        ...
        if (!cursor.find(rev)) {
          cursor = getSearchCursor(
            ...
            state.query,
            rev
              ? CodeMirror$1.Pos(cm.lastLine())
              : CodeMirror$1.Pos(cm.firstLine(), 0),
          )
  ...
  ;(function (mod) {
    ...
    CodeMirror$1.defineMode('css', function (config, parserConfig) {
      ...
      if (!parserConfig.propertyKeywords)
        parserConfig = CodeMirror$1.resolveMode('text/css')
      ...
      states.top = function (type$1, stream, state) {
        ...
        else if (/^@(-moz-)?document$/i.test(type$1))
        ...
        } else if (/^@(-(moz|ms|o|webkit)-)?keyframes$/i.test(type$1))
          return 'keyframes'
        else if (type$1 && type$1.charAt(0) == '@')
        ...
        else if (type$1 == 'interpolation')
          return pushContext(state, stream, 'interpolation')
        else if (type$1 == ':') return 'pseudo'
        else if (allowNested && type$1 == '(')
      ...
      states.documentTypes = function (type$1, stream, state) {
        if (
          type$1 == 'word' &&
          documentTypes$1.hasOwnProperty(stream.current())
        ) {
          override = 'tag'
          return state.context.type
      ...
      states.atBlock = function (type$1, stream, state) {
        ...
        if (type$1 == 'word') {
          ...
          else if (mediaValueKeywords$1.hasOwnProperty(word))
            override = 'keyword'
          else if (propertyKeywords$1.hasOwnProperty(word))
            override = 'property'
          else if (nonStandardPropertyKeywords$1.hasOwnProperty(word))
            override = highlightNonStandardPropertyKeywords
              ? 'string-2'
      ...
      states.atBlock_parens = function (type$1, stream, state) {
        if (type$1 == ')') return popContext(state)
        if (type$1 == '{' || type$1 == '}')
          return popAndPass(type$1, stream, state, 2)
        ...
        if (type$1 == 'word') {
          if (
            (state.stateArg == '@font-face' &&
              !fontProperties$1.hasOwnProperty(
                stream.current().toLowerCase(),
              )) ||
            (state.stateArg == '@counter-style' &&
              !counterDescriptors$1.hasOwnProperty(
                stream.current().toLowerCase(),
              ))
          )
            override = 'error'
      ...
      states.interpolation = function (type$1, stream, state) {
        ...
        else if (type$1 != 'variable' && type$1 != '(' && type$1 != ')')
          override = 'error'
...
var require_simple = __commonJSMin((exports, module) => {
  ;(function (mod) {
      ...
      var mode = {
        ...
        copyState: function (state$1) {
          ...
          if (state$1.localState)
            s.localState = CodeMirror$1.copyState(
              state$1.local.mode,
              state$1.localState,
            )
    ...
    function enterLocalMode(config, state, spec, token) {
      ...
      var mode = pers
        ? pers.mode
        : spec.mode || CodeMirror$1.getMode(config, spec.spec)
      ...
      return {
        ...
        copyState: function (state) {
          return {
            ...
            inner:
              state.innerActive &&
              CodeMirror$1.copyState(state.innerActive.mode, state.inner),
            ...
            for (var i = 0; i < others.length; ++i) {
              ...
              if (found == stream.pos) {
                ...
                if (outer.indent) {
                  ...
                  if (possibleOuterIndent !== CodeMirror$1.Pass)
...
var require_handlebars = __commonJSMin((exports, module) => {
  ;(function (mod) {
    ...
    CodeMirror$1.defineMode('handlebars', function (config, parserConfig) {
      ...
      return CodeMirror$1.multiplexingMode(
        CodeMirror$1.getMode(config, parserConfig.base),
        {
          open: '{{',
          close: /\}\}\}?/,
          mode: handlebars,
    ...
    CodeMirror$1.defineMode('javascript', function (config, parserConfig) {
      ...
      function inScope(state, varname) {
        ...
        for (var cx$1 = state.context; cx$1; cx$1 = cx$1.prev)
          for (var v = cx$1.vars; v; v = v.next)
            if (v.name == varname) return true
      }
      function parseJS(state, style, type$1, content$1, stream) {
        ...
        while (true) {
          ...
          if (combinator(type$1, content$1)) {
            ...
            if (type$1 == 'variable' && inScope(state, content$1))
              return 'variable-2'
            return style
          }
        }
      ...
      function statement(type$1, value) {
        ...
        if (type$1 == 'class' || (isTS && value == 'interface')) {
          cx.marked = 'keyword'
          return cont(
            pushlex('form', type$1 == 'class' ? type$1 : value),
            className,
            poplex,
          )
        }
      ...
      function expressionInner(type$1, value, noComma) {
        ...
        if (type$1 == 'keyword c' || type$1 == 'async')
        ...
        if (type$1 == 'operator' || type$1 == 'spread')
          return cont(noComma ? expressionNoComma : expression)
        if (type$1 == '[')
      ...
      function typeexpr(type$1, value) {
        ...
        if (type$1 == 'string' || type$1 == 'number' || type$1 == 'atom')
          return cont(afterType)
        if (type$1 == '[')
          return cont(
            pushlex(']'),
      ...
      function typeprop(type$1, value) {
        ...
        } else if (value == '?' || type$1 == 'number' || type$1 == 'string')
          return cont(typeprop)
      ...
      function typearg(type$1, value) {
        if (
          (type$1 == 'variable' && cx.stream.match(/^\s*[?:]/, false)) ||
          value == '?'
        )
          return cont(typearg)
        if (type$1 == ':') return cont(typeexpr)
      ...
      function maybeelse(type$1, value) {
        if (type$1 == 'keyword b' && value == 'else')
      ...
      return {
        ...
        skipExpression: function (state) {
          parseJS(
            ...
            'atom',
            'true',
            new CodeMirror$1.StringStream('', 2, null),
          )
        },
      }
    })
      ...
      function (config, parserConfig) {
          ...
          if (
            ...
            state.token = function (stream$1, state$1) {
              ...
              return maybeBackup(
                stream$1,
                endTag,
                state$1.localMode.token(stream$1, state$1.localState),
              )
            }
            state.localMode = mode
            state.localState = CodeMirror$1.startState(
              mode,
        ...
        return {
          ...
          copyState: function (state) {
            var local
            if (state.localState)
              local = CodeMirror$1.copyState(state.localMode, state.localState)
            return {
              token: state.token,
              inTag: state.inTag,
  ...
  ;(function (mod) {
    ...
    CodeMirror$1.findModeByMIME = function (mime) {
      ...
      for (var i$1 = 0; i$1 < CodeMirror$1.modeInfo.length; i$1++) {
        ...
        if (info$1.mimes) {
          for (var j = 0; j < info$1.mimes.length; j++)
            if (info$1.mimes[j] == mime) return info$1
        }
      }
      if (/\+xml$/.test(mime))
        return CodeMirror$1.findModeByMIME('application/xml')
      if (/\+json$/.test(mime))
        return CodeMirror$1.findModeByMIME('application/json')
    }
    CodeMirror$1.findModeByExtension = function (ext) {
      ...
      for (var i$1 = 0; i$1 < CodeMirror$1.modeInfo.length; i$1++) {
        var info$1 = CodeMirror$1.modeInfo[i$1]
        if (info$1.ext) {
          for (var j = 0; j < info$1.ext.length; j++)
            if (info$1.ext[j] == ext) return info$1
        }
      }
    }
      ...
      function (cmCfg, modeCfg) {
        ...
        function blockNormal(stream, state) {
          ...
          if (
            ...
            if (state.localMode)
              state.localState = CodeMirror$1.startState(state.localMode)
        ...
        var mode = {
          ...
          copyState: function (s) {
            return {
              ...
              block: s.block,
              htmlState:
                s.htmlState && CodeMirror$1.copyState(htmlMode, s.htmlState),
              indentation: s.indentation,
              localMode: s.localMode,
              localState: s.localMode
                ? CodeMirror$1.copyState(s.localMode, s.localState)
                : null,
              inline: s.inline,
              text: s.text,
              formatting: false,
      ...
      function (config) {
        ...
        State.prototype.copy = function () {
          ...
          if (this.innerMode && this.innerState)
            res.innerState = CodeMirror$1.copyState(
              this.innerMode,
              this.innerState,
            )
        ...
        function filter(stream, state) {
          if (stream.match(/^:([\w\-]+)/)) {
            ...
            if (config && config.innerModes)
              innerMode$1 = config.innerModes(stream.current().substring(1))
            if (!innerMode$1) innerMode$1 = stream.current().substring(1)
            if (typeof innerMode$1 === 'string')
              innerMode$1 = CodeMirror$1.getMode(config, innerMode$1)
  ...
  ;(function (mod) {
    ...
    CodeMirror$1.defineMode('stylus', function (config) {
      ...
      states.block = function (type$1, stream, state) {
        ...
        if (typeIsInterpolation(type$1, stream))
          return pushContext(state, stream, 'interpolation')
        ...
        if (typeIsBlock(type$1, stream))
          return pushContext(state, stream, 'block')
        if (type$1 == '}' && endOfLine(stream))
          return pushContext(state, stream, 'block', 0)
        ...
        if (/@(-(moz|ms|o|webkit)-)?keyframes$/.test(type$1))
          return pushContext(state, stream, 'keyframes')
        if (/@extends?/.test(type$1))
        ...
        if (type$1 && type$1.charAt(0) == '@') {
          ...
          if (/(@import|@require|@charset)/.test(type$1))
            return pushContext(state, stream, 'block', 0)
          return pushContext(state, stream, 'block')
        }
        if (type$1 == 'reference' && endOfLine(stream))
          return pushContext(state, stream, 'block')
        if (type$1 == '(') return pushContext(state, stream, 'parens')
        if (type$1 == 'vendor-prefixes')
      ...
      states.atBlock = function (type$1, stream, state) {
        ...
        if (type$1 == 'operator' && /^(not|and|or)$/.test(stream.current()))
          override = 'keyword'
        return state.context.type
      }
      states.atBlock_parens = function (type$1, stream, state) {
...
function syncEditorScrolls(primary, target) {
  ...
  let x =
    ((tInfo.width - tInfo.clientWidth) / (pInfo.width - pInfo.clientWidth)) *
    pInfo.left
  let y =
    ((tInfo.height - tInfo.clientHeight) /
      (pInfo.height - pInfo.clientHeight)) *
    pInfo.top
...
var DiffEditor_vue_vue_type_script_setup_true_lang_default =
        ...
        return (
          ...
          createBlock(
            unref(Pe),
            {
              onResize:
                _cache[0] ||
                (_cache[0] = ($event) => onUpdate($event.prevPane.size)),
            },
            {
              default: withCtx(() => [
                withDirectives(
...
var FilepathItem_vue_vue_type_script_setup_true_lang_default =
  /* @__PURE__ */ defineComponent({
    __name: 'FilepathItem',
    props: {
      filepath: {},
    ...
    setup(__props) {
      ...
      async function openInEditor() {
        await fetch(
          `/__open-in-editor?file=${encodeURI(props.filepath)}:${props.line}:${props.column}`,
        )
      }
          ...
          createElementBlock(
            ...
            [
              createBaseVNode(
                'span',
                _hoisted_1$2,
                toDisplayString(unref(display)[0]),
                1,
              ),
              createBaseVNode(
                'span',
                _hoisted_2$2,
                toDisplayString(unref(display)[1]),
                1,
              ),
              props.line != null && props.column != null
                ? (openBlock(),
                  createElementBlock(
                    ...
                    _hoisted_3$2,
                    ':' +
                      toDisplayString(props.line) +
                      ':' +
                      toDisplayString(props.column),
                    1,
                  ))
...
var FilepathItem_default =
  FilepathItem_vue_vue_type_script_setup_true_lang_default
const _hoisted_1$1 = {
  'of-auto': '',
  p4: '',
  'font-mono': '',
...
var ErrorDisplay_vue_vue_type_script_setup_true_lang_default =
  /* @__PURE__ */ defineComponent({
    __name: 'ErrorDisplay',
    props: { error: {} },
    setup(__props) {
        ...
        return (
          ...
          createElementBlock('div', _hoisted_1$1, [
            ...
            createBaseVNode(
              'pre',
              _hoisted_2$1,
              toDisplayString(_ctx.error.message),
              1,
            ),
            _cache[1] ||
...
var ErrorDisplay_default =
  ErrorDisplay_vue_vue_type_script_setup_true_lang_default
...
var module_vue_vue_type_script_setup_true_lang_default =
  /* @__PURE__ */ defineComponent({
    __name: 'module',
    async setup(__props) {
      ...
      function getModuleId(fullPath) {
        ...
        return (
          new URL(fullPath, 'http://localhost').searchParams.get('id') || void 0
        )
      }
      const options = useOptionsStore()
      ...
      const info = ref(
        id.value
          ? (([__temp, __restore] = withAsyncContext(() =>
              rpc.getModuleTransformInfo(payload.query, id.value),
            )),
            (__temp = await __temp),
            __restore(),
            __temp)
      ...
      const currentIndex = computed(
        () =>
          (index.value != null ? +index.value : null) ??
          (info.value?.transforms.length || 1) - 1,
      )
      const deps = computed(() => {
        return mod.value?.deps
          .map((dep) => payload.modules.find((m) => m.id === dep))
          .filter(Boolean)
      })
      ...
      const transforms = computed(() => {
        ...
        return trs.map((tr, index$1) => ({
          ...
          noChange:
            !!tr.result &&
            index$1 > 0 &&
            tr.result === trs[index$1 - 1]?.result,
      ...
      async function refetch(clear = false) {
        if (id.value)
          info.value = await rpc.getModuleTransformInfo(
            payload.query,
            id.value,
            clear,
          )
      ...
      const currentTransform = computed(() =>
        transforms.value?.find((tr) => tr.index === currentIndex.value),
      )
      ...
      const sourcemaps = computed(() => {
        ...
        if (typeof sourcemaps$1 === 'string')
          sourcemaps$1 = safeJsonParse(sourcemaps$1)
        if (!sourcemaps$1?.mappings) return
        if (
          sourcemaps$1 &&
          !sourcemaps$1.sourcesContent?.filter(Boolean)?.length
        )
          sourcemaps$1.sourcesContent = [from.value]
        if (sourcemaps$1 && !sourcemaps$1.sources?.filter(Boolean)?.length)
          sourcemaps$1.sources = ['index.js']
                ...
                default: withCtx(() => [
                  ...
                  createBaseVNode(
                    'button',
                    {
                      'icon-btn': '',
                      'text-lg': '',
                      title: unref(sourcemaps)
                        ? 'Inspect sourcemaps'
                        : 'Sourcemap is not available',
                      ...
                      createBaseVNode(
                        'span',
                        {
                          ...
                          class: normalizeClass(
                            unref(sourcemaps) ? 'opacity-100' : 'opacity-25',
                          ),
                        },
                        null,
                        2,
                  ...
                  createBaseVNode(
                    'button',
                    {
                      ...
                      onClick:
                        _cache[1] ||
                        (_cache[1] = ($event) =>
                          (unref(options).view.lineWrapping =
                            !unref(options).view.lineWrapping)),
                    },
                    [
                      createBaseVNode(
                ...
                ? (openBlock(),
                  createElementBlock('div', _hoisted_10, [
                    createBaseVNode('div', null, [
                      _cache[15] ||
                        (_cache[15] = createTextVNode(
                          'No transform data for this module in the ',
                          -1,
                        )),
                      createVNode(
                        _component_Badge,
                          ...
                          createVNode(
                            unref(Pe),
                            {
                              ...
                              onResize:
                                _cache[7] ||
                                (_cache[7] = ($event) =>
                                  (unref(options).view.panelSizeModule =
                                    $event.prevPane.size)),
                            },
                            {
                              default: withCtx(() => [
