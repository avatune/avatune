...
function fileSaverSaveAs(blob, name, opts, popup) {
  ...
  const isSafari =
    /constructor/i.test(String(_global$1.HTMLElement)) || 'safari' in _global$1
...
function useSSRWidth() {
  const ssrWidth = hasInjectionContext()
    ? /* @__PURE__ */ injectLocal(ssrWidthSymbol, null)
    : null
...
function useStorage(key, defaults, storage, options = {}) {
  ...
  function update(event) {
    ...
    try {
      ...
      if (
        event === void 0 ||
        (event == null ? void 0 : event.newValue) !== serializedData
      )
...
function createBirpc(functions, options) {
  ...
  function close(error) {
    ...
    rpcPromiseMap.forEach(({ reject, method }) => {
      reject(
        error || new Error(`[birpc] rpc is closed, cannot call "${method}"`),
      )
    })
    rpcPromiseMap.clear()
    off(onMessage)
...
function createRPCClient(name, hot$1, functions = {}, options = {}) {
  ...
  const promise = Promise.resolve(hot$1).then((r) => {
    if (!r)
      console.warn(
        '[vite-hot-client] Received undefined hot context, RPC calls are ignored',
      )
    return r
  })
...
function createStaticRpcClient() {
  async function getModuleTransformInfo(query, id) {
    ...
    return fetchJson(
      `./reports/${query.vite}-${query.env}/transforms/${hash(id)}.json`,
    )
  }
  return {
    getMetadata: () => fetchJson('./reports/metadata.json'),
    getModulesList: (query) =>
      fetchJson(`./reports/${query.vite}-${query.env}/modules.json`),
    getPluginMetrics: (query) =>
      fetchJson(`./reports/${query.vite}-${query.env}/metric-plugins.json`),
    getModuleTransformInfo,
    resolveId: (query, id) =>
      getModuleTransformInfo(query, id).then((r) => r.resolvedId),
...
const isStaticMode =
  document.body.getAttribute('data-vite-inspect-mode') === 'BUILD'
const hot = createHotContext(
  '/___',
  `${location.pathname.split('/__inspect')[0] || ''}/`.replace(/\/\//g, '/'),
)
...
function inspectSourcemaps({ code, sourcemaps }) {
  ...
  window.open(
    `https://evanw.github.io/source-map-visualization#${serialized}`,
    '_blank',
  )
}
...
const usePayloadStore = defineStore('payload', () => {
  ...
  async function init() {
    ...
    if (
      !metadata.value.instances.some(
        (i) =>
          i.vite === query.value.vite &&
          i.environments.includes(query.value.env),
      )
    ) {
      const instance$1 = metadata.value.instances.find(
        (i) => i.vite === query.value.vite,
      )
      if (instance$1) query.value.env = instance$1.environments[0] || 'client'
      else
        query.value.env =
          metadata.value.instances[0].environments[0] || 'client'
    }
    await doQuery()
    watch(
      query,
  ...
  const instance = computed(() =>
    metadata.value.instances.find((i) => i.vite === query.value.vite),
  )
