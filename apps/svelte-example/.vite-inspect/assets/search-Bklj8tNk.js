    ...
    if (!path$1[index]) list.push(obj$1)
      ...
      if (
        ...
        for (let i = 0, len = value.length; i < len; i += 1)
          deepGet(value[i], path$1, index + 1)
...
function parse(query, options, { auto = true } = {}) {
  const next = (query$1) => {
    ...
    if (!isQueryPath && keys.length > 1 && !isExpression(query$1))
      return next(convertToExplicit(query$1))
    if (isLeaf(query$1)) {
    ...
    if (state.search.exactSearch)
      return modules.filter(
        (item) =>
          ...
          item.plugins.some((plugin) =>
            plugin.name.includes(state.search.text),
          ),
      )
    else {
