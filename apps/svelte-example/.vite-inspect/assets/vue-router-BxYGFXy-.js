...
function parseURL(parseQuery$1, location$1, currentLocation = '/') {
  ...
  if (searchPos > -1) {
    ...
    searchString = location$1.slice(
      searchPos + 1,
      hashPos > -1 ? hashPos : location$1.length,
    )
...
function useHistoryListeners(base, historyState, currentLocation, replace) {
  ...
  function beforeUnloadListener() {
    ...
    history$1.replaceState(
      assign({}, history$1.state, { scroll: computeScrollPosition() }),
      '',
    )
  }
  function destroy() {
...
function useHistoryStateNavigation(base) {
  ...
  function changeLocation(to, state, replace$1) {
    ...
    const url =
      hashIndex > -1
        ? (location$1.host && document.querySelector('base')
            ? base
...
;(function (NavigationFailureType$1) {
   ...
   */
  NavigationFailureType$1[(NavigationFailureType$1['cancelled'] = 8)] =
    'cancelled'
  /**
   * A duplicated navigation is a navigation that failed because it was
   * initiated while already being at the exact same location.
   */
  NavigationFailureType$1[(NavigationFailureType$1['duplicated'] = 16)] =
    'duplicated'
      ...
      if (token.type === 0) {
        ...
        if (re$1 !== BASE_PARAM_PATTERN) {
          ...
          try {
            ...
            throw new Error(
              `Invalid custom RegExp for param "${value}" (${re$1}): ` +
                err.message,
            )
          }
        }
...
function createRouter(options) {
  ...
  function resolve(rawLocation, currentLocation) {
    ...
    if (rawLocation.path != null)
      ...
      for (const key in targetParams)
        if (targetParams[key] == null) delete targetParams[key]
      matcherLocation = assign({}, rawLocation, {
          ...
          if (
            ...
            return pushWithRedirect(
              ...
              assign({ replace: replace$1 }, locationAsObject(failure$1.to), {
                state:
                  typeof failure$1.to === 'object'
                    ? assign({}, data, failure$1.to.state)
                    : data,
                force,
              }),
  ...
  function markAsReady(err) {
    if (!ready) {
      ...
      readyHandlers
        .list()
        .forEach(([resolve$1, reject]) => (err ? reject(err) : resolve$1()))
      readyHandlers.reset()
    }
    return err
  }
