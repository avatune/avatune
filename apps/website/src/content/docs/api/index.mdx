...
`AsyncImage` cannot decode SVG, so point it at the PNG endpoint:
...
struct RemoteAvatar: View {
    ...
    private var url: URL? {
        URL(
            string:
                "https://www.avatune.dev/api/png/?theme=\(theme)&seed=\(userID)&size=\(Int(size))"
        )
    }
...
For avatars without a network round trip, render them on-device with the [Swift package](/packages/swift) instead — it produces the same output for the same seed.
