...
| Format | Endpoint |
| ------ | -------- |
| SVG | `https://avatune.dev/api/svg` |
| PNG | `https://avatune.dev/api/png` |
...
```bash
GET https://avatune.dev/api/svg?theme=nevmstas&hair=short
```

### PNG Request

```bash
GET https://avatune.dev/api/png?theme=nevmstas&hair=short
```
...
```html
<img src="https://avatune.dev/api/svg?theme=yanliu&seed=user123" alt="Avatar" />
```
...
```html
<img
  src="https://avatune.dev/api/svg?theme=yanliu&seed=user-123"
  alt="User Avatar"
  width="200"
  height="200"
/>
...
```javascript
const avatarUrl = `https://avatune.dev/api/svg?theme=nevmstas&seed=${userId}&size=300`;
...
function UserAvatar({ userId, theme = "yanliu", size = 200 }) {
  const avatarUrl = `https://avatune.dev/api/svg?theme=${theme}&seed=${userId}&size=${size}`;
...
const avatarUrl = computed(
  () =>
    `https://avatune.dev/api/svg?theme=${props.theme}&seed=${props.userId}&size=${props.size}`
);
</script>
```
...
.avatar {
  width: 200px;
  height: 200px;
  background-image: url("https://avatune.dev/api/svg?theme=miniavs&seed=user-456");
...
const url = `https://avatune.dev/api/svg?${params}`;
```
