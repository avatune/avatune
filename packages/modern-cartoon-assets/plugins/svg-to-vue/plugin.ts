...
export const pluginSvgToVue = (options: PluginOptions = {}): RsbuildPlugin => ({
  name: PLUGIN_NAME,
  pre: ['rsbuild:svgr'],
  setup(api) {
    api.modifyBundlerChain((chain, { CHAIN_ID, environment }) => {
      ...
      try {
        if (chain.module.rules.has(CHAIN_ID.RULE.SVG)) {
          ...
          if (builtin?.exclude && typeof builtin.exclude.add === 'function') {
            builtin.exclude.add(/\?vue$/)
            if (debug)
              console.log(
                `[${PLUGIN_NAME}] added exclude ?vue to builtin svg rule`,
              )
          }
        }
      } catch (e) {
      ...
      try {
        ...
        if (debug)
          console.warn(`[${PLUGIN_NAME}] failed to add svg-vue oneOf:`, e)
      }
