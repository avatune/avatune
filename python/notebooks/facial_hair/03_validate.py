...
def _(mo):
    mo.md(r"""
    # Facial Hair Model Validation

    Loads `best_facial_hair_model.keras` and evaluates on the validation split.
...
def _(Path):
    ...
    MODEL_PATH = Path('./models/facial_hair/best_facial_hair_model.keras')
    OUTPUT_PATH = Path('./models/facial_hair')
    ...
    return CLASSES, DATA_PATH, IMG_SIZE, MODEL_PATH, OUTPUT_PATH
...
@app.cell
def _(CLASSES, X_val, classification_report, model, np, y_val):
    ...
    for idx, cls in enumerate(CLASSES):
        ...
        if mask.sum() > 0:
            ...
            per_class_acc[cls] = round(class_acc, 4)
            print(f'  {cls:12s}: {class_acc * 100:5.1f}% (n={mask.sum()})')
