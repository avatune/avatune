...
def _(mo):
    mo.md(r"""
    # Hair Length Model Validation

    Loads `best_hair_length_model.keras` and evaluates on the validation split.
...
def _(Path):
    ...
    MODEL_PATH = Path('./models/hair_length/best_hair_length_model.keras')
    OUTPUT_PATH = Path('./models/hair_length')
    ...
    return DATA_PATH, HAIR_LENGTHS, IMG_SIZE, MODEL_PATH, OUTPUT_PATH
...
@app.cell
def _(HAIR_LENGTHS, X_val, classification_report, model, np, y_val):
    ...
    per_class_acc = {}
    for idx, cls_name in enumerate(HAIR_LENGTHS):
        mask = y_val == idx
        if mask.sum() > 0:
            class_acc = float((y_pred[mask] == idx).sum() / mask.sum())
            per_class_acc[cls_name] = round(class_acc, 4)
            print(f'  {cls_name:10s}: {class_acc * 100:5.1f}% (n={mask.sum()})')
...
@app.cell
def _(HAIR_LENGTHS, confusion_matrix, np, plt, sns, y_pred, y_val):
